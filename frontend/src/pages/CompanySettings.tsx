...
export default function CompanySettingsPage() {
  ...
  const existingLogoId = company?.logoStorageId
  const existingLogoUrl = useQuery(
    api.storage.getLogoUrl,
    existingLogoId ? { storageId: existingLogoId } : "skip"
  )
  ...
  const handleSaveLogo = async () => {
    ...
    try {
      ...
      await saveLogo({ logoStorageId: storageId })
      setLogoFile(null)
    } catch (err: any) {
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-5 gap-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input placeholder="Ohana Clean Industria e Comercio LTDA" className="pl-10 h-11 rounded-xl" defaultValue="Ohana Clean Industria e Comercio LTDA" />
                    </div>
                  </div>
                  <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-x-5 gap-y-4">
                  <div className="sm:col-span-2 space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input placeholder="Rua, Avenida, etc" className="pl-10 h-11 rounded-xl" defaultValue="Rua das Palmeiras, 123" />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Numero</Label>
                    <Input placeholder="123" className="h-11 rounded-xl" defaultValue="123" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">CEP</Label>
                    <Input placeholder="00000-000" className="h-11 rounded-xl" defaultValue="01234-567" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Bairro</Label>
                    <Input placeholder="Bairro" className="h-11 rounded-xl" defaultValue="Jardim Industrial" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Cidade / UF</Label>
                    <div className="grid grid-cols-3 gap-2">
                      <Input placeholder="Cidade" className="col-span-2 h-11 rounded-xl" defaultValue="Sao Paulo" />
                      <Input placeholder="SP" className="h-11 rounded-xl text-center" defaultValue="SP" />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Complemento</Label>
                    <Input placeholder="Complemento" className="h-11 rounded-xl" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Pais</Label>
                    <Input placeholder="Brasil" className="h-11 rounded-xl" defaultValue="Brasil" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-border bg-card shadow-card p-6 flex flex-col items-center gap-4">
              ...
              >
                ...
                {logoSrc ? (
                  ...
                  <img src={existingLogoUrl} alt="Logo" className="h-20 w-20 rounded-2xl object-contain border border-border" />
                ) : (
              ...
              <Button onClick={logoFile ? handleSaveLogo : handleSave} disabled={loading} className="w-full rounded-xl shadow-primary-btn h-11 mt-auto">
                {loading ? "Salvando..." : logoFile ? "Salvar Logo" : "Salvar Alteracoes"}
              </Button>
            </div>
          </div>
        </section>
